...
// Load VIP data from vip.json file
...
// Get the sender's user ID
        ...
        // Check if the sender is a VIP member
        if (!isVip(event.senderID)) {
            // If not a VIP member, send a message indicating insufficient permissions
            api.sendMessage(
                "Sorry, you are not a VIP member. Please contact the admin(s) to access VIP commands.",
                event.threadID,
                event.messageID
            );
            // Exit the function
